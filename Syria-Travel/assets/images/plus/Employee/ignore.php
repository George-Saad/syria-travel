...
try{
    ...
    $query= "UPDATE reservations SET r_cancel_request='', r_employee_id = " . $_SESSION['e_id'] . " WHERE r_id=" . $r_id ;
    ...
    $event_details = 'your employee ignore cancel resevation to passenger  ' . $passenger_name .  ' on trip   ' . $f_id . 'for this reason :' . $m_details;
    $query = "insert into event_history (`event_id`, `event_date`, `event_time`, `event_action`, `event_details`, `event_actor_id`) 
              VALUES (NULL, CURRENT_DATE(), CURRENT_TIME(), 'igrone_cancel_reservation', '" . $event_details . "', '" . $_SESSION['e_id'] . "')";
    $result= mysqli_query($conn,$query);
    mysqli_free_result($result);

		$m_details="ignore cancel";
  ...
  curl_setopt($ch,CURLOPT_POSTFIELDS,$fcmdata);
    ...
    header("Location: canceling-reservation.php");
}catch (Exception $e){
    die("Error In Query");
}
?>