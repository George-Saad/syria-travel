...
try{
    $r_id= $_GET['r_id'];
    $query= "UPDATE reservations SET r_status='canceled', r_employee_id = " . $_SESSION['e_id'] . " WHERE r_id=" . $r_id ;
    ...
    $event_details = 'your employee accepted to cancel resevation to passenger  ' . $passenger_name .  ' on trip   ' . $f_id;
    $query = "insert into event_history (`event_id`, `event_date`, `event_time`, `event_action`, `event_details`, `event_actor_id`) 
              VALUES (NULL, CURRENT_DATE(), CURRENT_TIME(), 'cancel_reservation', '" . $event_details . "', '" . $_SESSION['e_id'] . "')";
    $result = mysqli_query($conn,$query);
